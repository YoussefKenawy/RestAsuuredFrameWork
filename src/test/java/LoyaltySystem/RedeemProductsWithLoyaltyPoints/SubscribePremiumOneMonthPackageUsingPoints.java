...
public class SubscribePremiumOneMonthPackageUsingPoints extends BaseTest
    {
    ...
    public void redeemPointForSubscribePremiumOneMonthPackage() throws InterruptedException
        {
            Map<String, Object> requestBody = Map.of("packageName", "PREMIUM_ONE_MONTH", "paymentMethod", "LOYALTY_CREDIT");
            String endpoint = "/subscribe-package";
            Response response = performPatch(endpoint, newReaToken, requestBody, sendHeaders());
            Assert.assertEquals(response.getStatusCode(), 200);
            Assert.assertEquals(response.jsonPath().getString("data.subscription.packageName"), "PREMIUM_ONE_MONTH");
