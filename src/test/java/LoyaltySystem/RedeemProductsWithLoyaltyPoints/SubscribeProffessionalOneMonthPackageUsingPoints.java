...
public class SubscribeProffessionalOneMonthPackageUsingPoints extends BaseTest
    {
    ...
    public void redeemPointForSubscribePremiumOneMonthPackage() throws InterruptedException
        {
            Thread.sleep(5000);
            Map<String, Object> requestBody = Map.of("packageName", "PROFESSIONAL_ONE_MONTH", "paymentMethod", "LOYALTY_CREDIT");
            String endpoint = "/subscribe-package";
            Response response = performPatch(endpoint, newReaToken, requestBody, sendHeaders());
            Assert.assertEquals(response.getStatusCode(), 200);
            Assert.assertEquals(response.jsonPath().getString("data.subscription.packageName"), "PROFESSIONAL_ONE_MONTH");
